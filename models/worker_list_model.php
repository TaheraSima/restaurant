<?php
//Worker_List Models
class Worker_List_Model extends Model
{
	...
	function update(){
		...
		$upstmt = $this->db->prepare("UPDATE `users` SET `full_name` = '$full_name', `username` = '$username', `user_type`='$user_type' WHERE `user_id`=$user_id_md5 ");
		if ( $upstmt->execute() === TRUE ) {
			return 'SUCCESS';
		}else{
			return 'FAILED';
