    ...
    <div class="modal-content">
      ...
      <form action="<?php echo url('new_purchase/save'); ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="modal-body mt-2">
          ...
          <div class="col-md-4">
              <button style="font-size: 15px; " type="button" class="btn btn-success item-add">+ More Material</button>
          </div><br><br>
          <table class="table table-sm table-responsive">
              
                    <thead style="text-align: center;">
                      <tr style="background-color: #4ab300; font-size: 15px; color: white">
                      <td>#</td>
                      <td>Item Name</td>
...
<script>
    ...
    var closingqtyElem = quantityElem.parent().next().next().next().find( '.closing_qty' );
      $('.quantity').each(function(){
        var purchase_qty = parseFloat( quantityElem.val() );
        var prevqnty = parseFloat( prevElem.val() );
        var c_qty = purchase_qty + prevqnty;
        var currentqty = parseFloat(c_qty);
         closingqtyElem.val(currentqty);
  }); 
}); 
